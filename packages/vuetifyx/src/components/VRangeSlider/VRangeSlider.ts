...
export default VSlider.extend({
  ...
  computed: {
    ...
    trackFillStyles () {
      ...
      const start = this.vertical ? this.$vuetifyx.rtl ? 'top' : 'bottom' : this.$vuetifyx.rtl ? 'right' : 'left'
  ...
  methods: {
    getTrackStyle (startLength: number, endLength: number, startPadding = 0, endPadding = 0) {
      const startDir = this.vertical ? this.$vuetifyx.rtl ? 'top' : 'bottom' : this.$vuetifyx.rtl ? 'right' : 'left'
    ...
    genTrackContainer () {
      ...
      if (this.disabled) {
        ...
        if (this.$vuetifyx.rtl) sections.reverse()
