...
>(
  ...
  computed: {
    ...
    trackFillStyles() {
      ...
      const start = this.$vuetifyx.rtl ? 'auto' : 0
      const end = this.$vuetifyx.rtl ? 0 : 'auto'
    ...
    trackStyles() {
      const startDir = this.vertical ? (this.$vuetifyx.rtl ? 'bottom' : 'top') : this.$vuetifyx.rtl ? 'left' : 'right'
  ...
  methods: {
    ...
    genChildren(): VNodeChildrenArrayContents {
      return [
        ...
        this.genSteps(),
        this.genThumbContainer(this.internalValue, this.inputWidth, this.isActive, this.isFocused, this.onThumbMouseDown, this.onFocus)
      ]
    },
    genInput(): VNode {
    ...
    genSteps(): VNode | null {
      ...
      const ticks = range.map(i => {
        const index = this.$vuetifyx.rtl ? this.maxValue - i : i
        const children = []
        ...
        const filled = this.$vuetifyx.rtl ? 100 - this.inputWidth < width : width < this.inputWidth
    ...
    getThumbContainerStyles(width: number): object {
      ...
      let value = this.$vuetifyx.rtl ? 100 - width : width
    ...
    parseMouseMove(e: MouseEvent) {
      ...
      const { [start]: trackStart, [length]: trackLength } = this.$refs.track.getBoundingClientRect() as any
      ...
      if (this.$vuetifyx.rtl) clickPos = 1 - clickPos
      ...
      if (this.disabled) {
        ...
        if (![pageup, pagedown, end, home, left, right, down, up].includes(e.keyCode)) {
          ...
          if ([left, right, down, up].includes(e.keyCode)) {
            this.keyPressed += 1
            const increase = this.$vuetifyx.rtl ? [left, up] : [right, up]
