  ...
  computed: {
    ...
    isMobile (): boolean {
      return !this.stateless &&
        !this.permanent &&
        !this.temporary &&
        this.$vuetifyx.breakpoint.width < parseInt(this.mobileBreakPoint, 10)
    },
    marginTop (): number {
      if (!this.hasApp) return 0

      let marginTop = this.$vuetifyx.application.bar

      marginTop += this.clipped
        ? this.$vuetifyx.application.top
        : 0
    ...
    maxHeight (): number | null {
      ...
      const maxHeight = (
        this.$vuetifyx.application.bottom +
        this.$vuetifyx.application.footer +
        this.$vuetifyx.application.bar
      )

      if (!this.clipped) return maxHeight

      return maxHeight + this.$vuetifyx.application.top
    },
