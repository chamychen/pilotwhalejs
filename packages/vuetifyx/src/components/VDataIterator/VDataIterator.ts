...
export default mixins(Themeable).extend({
  ...
  props: {
    ...
    noResultsText: {
      type: String,
      default: '$vuetifyx.dataIterator.noResultsText'
    },
    noDataText: {
      type: String,
      default: '$vuetifyx.noDataText'
    },
    loadingText: {
      type: String,
      default: '$vuetifyx.dataIterator.loadingText'
    },
  ...
  methods: {
    ...
    genEmpty (itemsLength: number) {
      if (itemsLength <= 0 && this.loading) {
        const loading = this.$slots['loading'] || this.$vuetifyx.lang.t(this.loadingText)
        return this.genEmptyWrapper(loading)
      } else if (itemsLength <= 0 && !this.items.length) {
        const noData = this.$slots['no-data'] || this.$vuetifyx.lang.t(this.noDataText)
        return this.genEmptyWrapper(noData)
      } else if (itemsLength <= 0 && this.search) {
        const noResults = this.$slots['no-results'] || this.$vuetifyx.lang.t(this.noResultsText)
