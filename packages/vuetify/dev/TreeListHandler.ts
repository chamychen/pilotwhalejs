...
export default class TreeListHandler {
    ...
    private reCalculateLongCode(item: any, newLongCode: string): Array<any> | null {
        if (item && !stringUtils.isEmpty(newLongCode)) {
            ...
            allItems.forEach(thisItem => {
                ...
                let regex = new RegExp(`^${thisOldLongCode}`)
                let thisNewLongCode = thisOldLongCode.replace(regex, newLongCode)
     ...
     * @param longCode 
     */
    private getDescendantItems(longCode: string) {
        if (stringUtils.isEmpty(longCode)) {
            return this._data
        } else {
     ...
     * @param longCode 
     */
    private getYoungerBrothers(longCode: string) {
        if (this._data) {
            if (stringUtils.isEmpty(longCode)) {
